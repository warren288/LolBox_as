...
public class BaseKitManager {
	 ...
	 * @param strUrl
	 */
	public static void openSummonerKey(BaseActivity activity, String strUrl){
		Intent it = new Intent(activity, BaseWebActivity.class);
		it.putExtra(BaseWebActivity.EXTRA_URL, strUrl);
		it.putExtra(BaseWebActivity.EXTRA_TITLE, "召唤师关键词");
		activity.startActivity(it);
	}
	...
	public static boolean openUrl(BaseActivity activity, String strUrl) {
		...
		/**
		 * 查看英雄评论
		 * http://comment3.duowan.com/index.php?r=phone/index&domain=lolbox.duowan.com&url=/heroDetail/Nunu&order=hot&color=&assest
		 */
		...
		} else if(strUrl.contains("comment3.duowan.com")){
			if(activity instanceof  BaseWebActivity){
				((BaseWebActivity)activity).loadUrl(strUrl);
			}else{
				BaseKitManager.openUrl(activity, strUrl, "评论");
			}
		}else if(strUrl.contains("http://service.mbox.duowan.com/index.php?r=tags")) {
			BaseKitManager.openSummonerKey(activity, strUrl);
		} else{
			return false;
		}
		return true;
	 ...
	 * @param strTitle
	 */
	public static void openUrl(BaseActivity activity, String strUrl, String strTitle){
		Intent it = new Intent(activity, BaseWebActivity.class);
		it.putExtra(BaseWebActivity.EXTRA_URL, strUrl);
		it.putExtra(BaseWebActivity.EXTRA_TITLE, strTitle);
		activity.startActivity(it);
	}
}
