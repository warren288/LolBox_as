...
public class HeroVideoActivity extends BaseActivity {
	...
	private void requestData() {
		...
		AppContext.getApp().getNetManager()
					.get(URLUtil.getURL_Video(mStrNameEng, 1), map, new IListener<String>() {
						...
						public void onCall(String strJson) {
							...
							AppContext.getApp()
										.getJsonManager()
										.parseList(strJson, Video.class,
													new IListener<List<Video>>() {
														...
														public void onCall(List<Video> t) {
															...
															mLvVideos.setSelection(mLstVideos
																		.size());
														}
	...
	private void requestMore() {
		...
		httpGet(URLUtil.getURL_Video(mStrNameEng, mCount + 1), map, new IListener<String>() {

			@Override
			public void onCall(String strJson) {

				jsonParseList(strJson, Video.class, new IListener<List<Video>>() {
